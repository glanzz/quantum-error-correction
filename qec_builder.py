...
class QECBuilder:
  def __init__(self, distance=5, repeat=5,  x_error=0.001, depolarize_error=0.001):
    assert distance > 0
    self.distance = (distance*2)-1
    self.x_error = x_error
    self.depolarize_error = depolarize_error
    self.repeat = repeat
  ...
  def build_cx_layer(self):
    ...
    self.circuit.depolarize(self.depolarize_error, cx_qubits)
    self.circuit.depolarize(self.depolarize_error, [self.distance-1], single_qubit=True) # 1q depolarize
    self.circuit.tick()
    ...
    self.circuit.cx(cx_qubits)
    self.circuit.depolarize(self.depolarize_error, cx_qubits)
    self.circuit.depolarize(self.depolarize_error, [0], single_qubit=True)
    self.circuit.tick()
  
  def measure_non_data(self, non_data_qubits):
    self.circuit.x_error(self.x_error, non_data_qubits)
  ...
  def __initalize_build(self):
    ...
    self.circuit.x_error(self.x_error, range(self.distance))
    self.circuit.tick()
  
  def __build_core_layer(self):
    self.build_cx_layer()
    self.measure_non_data(self.non_data_qubits)
    self.circuit.depolarize(self.depolarize_error, self.data_qubits, single_qubit=True)
  ...
  def __build_repitition_layer(self):
      ...
      self.circuit.x_error(self.depolarize_error, self.data_qubits)
  ...
  def __build_final_layer(self):
      self.circuit.x_error(self.x_error, self.data_qubits)
  ...
  def build(self):
    ...
    self.circuit.repeat(self.repeat, self.__build_repitition_layer)
